...
export async function cancelPayment(client: DocumentClient, paymentToken: string) : Promise<boolean | null> {
    try {
        ...
        console.log({"message": "Error cancelling the paymentToken from the database", "errormsg": dbError});
        return null;
    }
}
...
export const handler = async (event: any = {}) : Promise <any> => {
    ...
    // Check token
    const result = await exports.cancelPayment(client, body.paymentToken);
    if (result === null) {
